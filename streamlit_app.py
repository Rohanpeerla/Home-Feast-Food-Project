...
def apply_styles():
    st.markdown(
        ...
        <style>
          .stApp {
            background:
              radial-gradient(circle at top left, rgba(200, 85, 45, 0.10), transparent 30%),
              radial-gradient(circle at top right, rgba(255, 197, 145, 0.16), transparent 24%),
              linear-gradient(180deg, #fff8ef 0%, #fffdf9 34%, #ffffff 100%);
            color: #2b211b;
          }
          .block-container {
            padding-top: 1.35rem;
...
def page_shell(title, subtitle=None, back_to=None):
    st.markdown(
        f"""
        <div class="hf-hero">
          <div class="hf-kicker">{APP_TITLE}</div>
          <div class="hf-brand" style="font-size:1.7rem;">{title}</div>
          {f'<div class="hf-sub">{subtitle}</div>' if subtitle else ''}
        </div>
        """,
        unsafe_allow_html=True,
    )
    nav_buttons(back_to=back_to)
...
def login_form():
    page_shell("Login", "Access your account or create a new one.", back_to=HOME_PAGE)
...
def register_form():
    page_shell("Register", "Create a HomeFeast account.", back_to=HOME_PAGE)
...
def profile_section(user):
    page_shell("Profile", "Update your account details.", back_to=HOME_PAGE)
...
def subscriptions_section(user):
    page_shell("Subscriptions", "Review your active and past plans.", back_to=HOME_PAGE)
...
def orders_section(user):
    page_shell("Orders", "See recent meals and delivery status.", back_to=HOME_PAGE)
...
def discover_section(user):
    page_shell("Discover Cooks", "Browse by cuisine, meal type, or location.", back_to=HOME_PAGE)
    st.markdown(
        """
        <div class="hf-card">
...
def cook_profile_section():
    ...
    page_shell("Cook Profile", "View meal details before you subscribe.", back_to="Discover Cooks")
...
def payment_section(user):
    page_shell("Payment", "Choose a plan and complete your subscription.", back_to="Discover Cooks")
...
def home_section(user):
    ...
    st.markdown(
        f"""
        <div class="hf-hero">
          <div class="hf-kicker">HomeFeast dashboard</div>
    ...
    nav_items = [
        ("Dashboard", HOME_PAGE),
        ("Discover", "Discover Cooks"),
        ("Subscriptions", "Subscriptions"),
        ("Orders", "Orders"),
    ]
    for col, (label, target) in zip(quick_cols, nav_items):
        with col:
    ...
    st.markdown("### Featured cooks")
    featured = list(get_all_cooks())[:6]
    if not featured:
        st.info("No approved cooks are available right now.")
        return

    card_columns = st.columns(3)
    for idx, cook in enumerate(featured):
        col = card_columns[idx % 3]
        with col:
            st.markdown(
                f"""
                <div class="cook-card compact">
                  <div class="cook-title">{cook['name']}</div>
                  <div class="cook-meta">{cook['cuisines']} | {cook['location']}</div>
                  <div style="color:#5d5147; margin-bottom:0.55rem;">{cook['description'] or 'Fresh homestyle meals, made to order.'}</div>
                  <div class="hf-pill">From {format_money(cook['price_from'])}</div>
                  <div class="hf-pill">{cook['meal_types'] or 'Meals'}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            b1, b2 = st.columns(2)
            if b1.button("View", key=f"home_view_{cook['id']}"):
                st.session_state["selected_cook_id"] = cook["id"]
                st.session_state["page"] = "Cook Profile"
                st.rerun()
            if b2.button("Order", key=f"home_order_{cook['id']}"):
                st.session_state["selected_cook_id"] = cook["id"]
                st.session_state["page"] = "Payment"
                st.rerun()
    ...
    steps = [
        ("1. Explore", "Browse cooks by cuisine, location, and meal type."),
        ("2. Subscribe", "Pick a daily, weekly, or monthly plan."),
        ("3. Enjoy", "We keep your meals and deliveries organized."),
    ]
        ...
        with col:
            st.markdown(
                ...
                <div class="cook-card">
                  ...
                  <div style="color:#6f5d50;">{body}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if not user:
        ...
        if action_cols[0].button("Log in", use_container_width=True):
            st.session_state["page"] = "Login"
            st.rerun()
        if action_cols[1].button("Create account", use_container_width=True):
...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state["page"] = st.radio(
            "Navigate",
            pages,
            index=pages.index(normalize_page_name(st.session_state["page"])) if normalize_page_name(st.session_state["page"]) in pages else 0,
        )

    user = current_user()
    page = normalize_page_name(st.session_state["page"])
