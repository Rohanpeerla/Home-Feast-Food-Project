...
def profile_section(user):
    ...
    with st.form("profile_form"):
        ...
        meal_pref = st.selectbox("Meal preference", ["Vegetarian", "Non-Vegetarian", "Both"], index=0)
        cuisine_pref = st.selectbox("Cuisine preference", ["South Indian", "North Indian", "Any"], index=2)
...
def discover_section(user):
    ...
    if st.button("Filter"):
        ...
        if q:
            cooks = [c for c in cooks if q.lower() in (c["name"] or "").lower() or q.lower() in (c["cuisines"] or "").lower() or q.lower() in (c["location"] or "").lower()]
        if meal_type:
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigate", ["Home", "Discover Cooks", "Subscriptions", "Orders", "Profile", "Login", "Register"], index=["Home", "Discover Cooks", "Subscriptions", "Orders", "Profile", "Login", "Register"].index(st.session_state["page"]))
